...
//pass in the words as a parameter
void reverse_words(char* line) {
  int len;
  char* buffer;
  ...
  while (read_pos >= 0) {
    //if index is blank, just copy it over
    if (line[read_pos] == ' ') {
      buffer[write_pos++] = line[read_pos--];
    }
    else {
      //if line[read_pos] is not space
      word_end = read_pos;
      //find the blank next blank
      while (read_pos >= 0 && line[read_pos] != ' ')
	read_pos--;
      
      //the index the start of the word is the last blank seen +1
      word_start = read_pos + 1;
      //copy the words over to the buffer array
